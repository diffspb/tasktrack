...
- **Дизайн / прототип агент** — работает над UI, визуальными прототипами, компонентами дизайн-системы. Может коммитить и пушить напрямую в `main` без подтверждения человека.
...
- Пушить или мержить напрямую в `main` (кроме исключений ниже)
- Делать force push (`--force`)
- Удалять ветки или теги
- Делать `git reset --hard`

**Исключения — можно коммитить и пушить прямо в `main` без подтверждения:**
- Правки документации (`docs/`, `*.md` в корне)
- Прототипы и дизайн: изменения в `frontend/src/` носящие визуальный / UI характер (новые компоненты, стили, прототипы экранов, правки дизайн-системы), когда агент явно специализируется на дизайне
...
design/new-dashboard      — UI-прототип или новый экран
design/component-library  — компоненты дизайн-системы
```
...
> Дизайн-агент: работает в ветке `design/...` или напрямую в `main`, пушит самостоятельно.
