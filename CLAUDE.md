...
Текущий этап: **реализация MVP, Этап 7 (Decision Process UI)**.
...
**Текущий этап:** 7 — Decision Process UI
...
Различаем два режима:

- **Фоновые / делегированные агенты** (Agent tool, scheduled/cron, ultra-review) — работают без человека в петле. Им разрешён только набор «безопасных» действий ниже.
- **Основной интерактивный агент сессии** — работает в диалоге с человеком. По умолчанию следует тем же правилам, но при явном подтверждении человека (обычно по завершении этапа или согласованной задачи) может мержить фиче-ветку в `main` и пушить `main`.

Агенты **могут (по умолчанию):**
...
Агенты **не могут без явного подтверждения человека:**
- Пушить или мержить напрямую в `main`
