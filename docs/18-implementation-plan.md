...
## Этап 8. Доводка и переключение на миграции

**Цель:** Готово к исследовательскому запуску.

**Что делается:**
1. Notifications badge (счётчик непрочитанных в шапке)
2. Дашборд `/users/me/tasks` с фильтрами
3. FTS-поиск через PostgreSQL tsvector (russian)
4. Error handling на фронте (toast по error code)
5. Loading/skeleton states
6. **Alembic:** `alembic init` → `autogenerate` → убрать `create_all` из lifespan
7. Разделить `reset_db.py` на `migrate` (схема) и `seed` (данные)

**Тесты:** поиск, badge count
...
| 8 | `mvp-research-launch` | Готово к запуску |
