...
**Soft-delete комментариев не фильтруется при листинге.**
`comment_service.py:85-86` — `list_comments` не добавляет `.where(Comment.deleted_at.is_(None))`. Удалённые комментарии видны клиенту в списке. Однострочный фикс.
